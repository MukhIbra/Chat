...
//                            if (keys.size == users.size) callback(users,lastMessages)
                    ...
                    getAllUsers{ u->
                        u.forEach{
                            Log.d("user", "onDataChange: ${it.username}")
                            if (!users.contains(it) && it.key != key){
                                Log.d("user false", "onDataChange: ${it.username}")
                                users.add(it)
                                lastMessages.add(Message(it.key,key,"","",""))
                            }
                        }
                        Log.d("full users1", "onDataChange: ${users.joinToString()}")
                        callback(users,lastMessages)
                    }
